...
export default function WeeklyGoalWidget() {
  const { t } = useTranslation('weeklyGoalWidget')
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm space-y-4">
      <div>
        ...
        <div className="h-2.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden"
          ...
          aria-label={t('progressAriaLabel', { percent: progress.prozent })}>
          <div
      ...
      {streak && streak.streak > 0 && (
        <div className="flex items-center gap-2 text-sm">
          ...
          <div>
            <p className="font-medium">{t('streak', { count: streak.streak })}</p>
            <p className="text-xs text-gray-400">{t('lastActivity', { date: streak.letzte_aktivitaet })}</p>
          </div>
        </div>
      )}
    </div>
