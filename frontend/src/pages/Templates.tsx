...
function GenerateModal({
  ...
  const generateMutation = useMutation({
    ...
    onError: (err: any) => {
      setError(err?.response?.data?.detail || err.message || 'Fehler bei der Generierung')
    },
  })
      ...
      <div>
        ...
        {uploadMutation.isError && (
          <p className="text-sm text-red-500 mt-2 flex items-center gap-1">
            <AlertCircle size={14} /> Fehler beim Hochladen: {(uploadMutation.error as any)?.response?.data?.detail || 'Unbekannter Fehler'}
          </p>
        )}
